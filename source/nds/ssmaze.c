...
uint8_t maze_desc[MAZE_ARRAY][MAZE_ARRAY];
...
/*
void (*convex_solids[SPECIAL_ARG_COUNT])(GLdouble radius) =
{
    auxSolidIcosahedron,
    auxSolidOctahedron,
...
void Render(MazeView *vw)
{
    ...
    glLoadIdentity();
    glRotatef(view_rot, 0, 0, 1);
    gluPerspective(VIEW_ANG, 1, 0.1f, 1000);
    viewHeight = 0.5f;
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
	glScalef32(inttof32(SCALE_VERTICES), inttof32(SCALE_VERTICES), inttof32(SCALE_VERTICES));
	gluLookAt(FxFlt(vw->pos.x / SCALE_VERTICES), FxFlt(vw->pos.y / SCALE_VERTICES), viewHeight / SCALE_VERTICES,
              FxFlt(at.x / SCALE_VERTICES), FxFlt(at.y / SCALE_VERTICES), viewHeight / SCALE_VERTICES,
              0, 0, 1);
...
uint32_t DrawMazeWallsList(void* list)
{
	uint32_t s = 0;
...
#define AA_LINES 0
    ...
    for (c = 0; c < MAZE_CELLS; c++)
    {
        for (obj = cell->contents; obj != NULL; obj = obj->next)
        {
            ...
            glRotatef(FaFltDegVal(obj->ang), 0, 0, 1);
